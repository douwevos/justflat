...
public class ContourLayerViewer extends ModelViewer<ContourLayerViewableModel> {
	...
	protected void paintModel(BufferedImage image, Graphics2D g, ContourLayerViewableModel model) {
		ContourLayer mainLayer = model.mainLayer;
		ContourLayer ghostLayer = model.ghostLayer;
		if (ghostLayer!=null) {
			Graphics2D g2d = (Graphics2D)g.create();
			g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.35f));
			paintModelLayer(image, g2d, ghostLayer);
			g2d.dispose();
		}
		
		paintModelLayer(image, g, mainLayer);
	}
	
	
	protected void paintModelLayer(BufferedImage image, Graphics2D g, ContourLayer modelLayer) {
...
//				g.drawLine((int) pointA.x,(int) pointA.y, (int) pointB.x,(int) pointB.y);
	...
	private void drawScanLine(Graphics2D gfx, ContourLayerViewableModel viewableModel) {
		if (viewableModel==null || viewableModel.mainLayer == null) {
			return;
		}
		...
		List<StartStop> startStop = viewableModel.mainLayer.scanlineHorizontal(scanLine);
		if (startStop == null) {
			return;
		}
...
//		g.drawLine(inx, iny, inx+py, iny+px);
	...
	public boolean onDrag(ModelMouseEvent event, Selection<?> selected) {
		return model==null ? false : model.mainLayer.dragTo(selected, event.modelX, event.modelY);
	}
